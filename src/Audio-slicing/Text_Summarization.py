...
def generate_summary(path):
    ...
    # Reading Data
    text = open(path,"r").read() # import text file extracted from audio

    # Cleaning Data
    text = re.sub(r'\[[0-9]*\]',' ',text)    
    text = re.sub(r'\s+',' ',text)
    clean_text = text.lower()
    clean_text = re.sub(r'\W',' ',clean_text)   # removes punctuation marks
    clean_text = re.sub(r'\d',' ',clean_text)   # removes numbers
    clean_text = re.sub(r'\s+',' ',clean_text)  # removes whitespaces
